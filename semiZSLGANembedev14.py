...
class Generat(nn.Module):
    def __init__(self, channel=64, dim_z=128, bottom_width=4, n_cls=0):
        ...
        # print(self.l1)
        self.b1 = GBlockEm(channel * 16, channel * 16, upsample=True, num_em=n_cls)
        self.b2 = GBlockEm(channel * 16, channel * 8, upsample=True, num_em=n_cls)
        self.b3 = GBlockEm(channel * 8, channel * 8, upsample=True, num_em=n_cls)
        self.b4 = GBlockEm(channel * 8, channel * 4, upsample=True, num_em=n_cls)
        self.b5 = GBlockEm(channel * 4, channel * 2, upsample=True, num_em=n_cls)
        self.b6 = GBlockEm(channel * 2, channel, upsample=True, num_em=n_cls)
        self.bn1 = nn.BatchNorm2d(channel)
        self.con1 = nn.Conv2d(channel, 3, kernel_size=3, stride=1, padding=1)
        self.actf = nn.ReLU(inplace=True)
        self.tanhf = nn.Tanh()

    def forward(self, z=None, y=None, bat_size=10, **kwargs):
        if z is None:
...
generator = Generat(channel, dim_z=noise_size, n_cls=40)  # channel,noise_size,4,0
...
for i_epoch in range(tr_epoch):
    for i_data, data in enumerate(data_container):
        ...
        output_d = discriminator(out_back, x_train_label)  # 判别器网络
        output_regr = regressor_net(out_back)  # 回归器网络
        # 生成样本的输出
        # print(noise.size())
        fake_img = generator(noise, x_train_label).detach()
        out_back_g = back_bone(fake_img)
        output_g = discriminator(out_back_g, x_train_label)  # 输入类标,x_ground_truth
        # 计算loss
        ...
        if (i_data + 1) % (5 * times_batch) == 0:
            for b_i_gan in range(times_batch):
                ...
                fake_pic = generator(noise, x_train_label)
                out_back_tg = back_bone(fake_pic)
                output_tg = discriminator(out_back_tg, x_train_label).mean().view(1)  # ,x_ground_truth
                # torch.backends.cudnn.enabled = False
    ...
    if (i_epoch + 1) % 2 == 0:
        ...
        outprint.save(
            './results/' + back_version + str(i_epoch + 1) + ' th ' + version_p + ' epoch embed semizslGanGAN.png')

    if (i_epoch + 1) % 50 == 0:
        torch.save(back_bone.state_dict(),
                   './models/' + back_version + str(i_epoch + 1) + ' th ' + version_p + ' embed semiGAN_backbone.pkl')
        torch.save(discriminator.state_dict(),
                   './models/' + back_version + str(
                       i_epoch + 1) + ' th ' + version_p + ' embed semiGAN_discriminator.pkl')
        torch.save(generator.state_dict(),
                   './models/' + back_version + str(i_epoch + 1) + ' th ' + version_p + ' embed semiGAN_generator.pkl')
        torch.save(regressor_net.state_dict(),
                   './models/' + back_version + str(
                       i_epoch + 1) + ' th ' + version_p + ' embed semiGAN_regressor_net.pkl')
